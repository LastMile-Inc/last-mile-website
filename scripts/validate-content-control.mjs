...
for (const filePath of scanRoots) {
  ...
  const dependencyPattern = /(?:service\s*now.{0,80}(?:required|requirement|foundation|prerequisite|dependency|privileged)|(?:requires?|depends? on|foundation|prerequisite|built on|must use).{0,80}service\s*now)/gi;
  for (const match of text.matchAll(dependencyPattern)) {
    ...
    if (!/(?:not required|does not depend|without depending|no privileged|none is|not a prerequisite|not dependent|not the foundation|no architectural dependency)/i.test(context)) fail(`${rel} describes ServiceNow as required or privileged: ${match[0]}`);
  }
}
...
for (const scenario of platformData.operatingScenarioList) {
  ...
  if (!scenario.processMap?.src?.startsWith("/images/reference/") || !fs.existsSync(path.join(root, "public", scenario.processMap.src))) fail(`${scenario.route} has a broken process-map path ${scenario.processMap?.src ?? "(missing)"}`);
