...
import { ArrowRight, Check, CircleAlert, CircleCheck, CircleHelp, CircleX, Expand, Info, ShieldCheck, TriangleAlert, X } from "lucide-react";
import {
  coolingReference,
  deriveRag,
...
export function ExpandableProcessMap({ scenario, textVersion }: { scenario: OperatingScenario; textVersion: ReactNode }) {
  ...
  return <section className="lm-process-map-card">
    <div><span>Canonical process map</span><h3>{scenario.menuLabel}</h3><p>The image supports the semantic operating response below. Open it at full technical scale; mobile users can pan without shrinking labels into illegibility.</p></div>
    <figure><img src={scenario.processMap.src} alt={scenario.processMap.alt} width="1920" height="1080" loading="lazy" /><figcaption>{scenario.processMap.alt}.</figcaption></figure>
    <button ref={triggerRef} type="button" onClick={() => setOpen(true)}>Expand technical process map <Expand aria-hidden="true" /></button>
    <div className="lm-process-map-text"><h3>Accessible text version</h3>{textVersion}</div>
    {open ? <dialog ref={dialogRef} className="lm-process-map-dialog" aria-label={`${scenario.menuLabel} process map`} onCancel={(event) => { event.preventDefault(); dialogRef.current?.close(); }}>
      <header><strong>{scenario.menuLabel}</strong><button type="button" onClick={() => dialogRef.current?.close()} aria-label="Close expanded process map"><X aria-hidden="true" /></button></header>
      <div className="lm-process-map-dialog__viewport"><img src={scenario.processMap.src} alt={scenario.processMap.alt} width="1920" height="1080" /></div>
      <p>Use horizontal and vertical scrolling to inspect the original-size reference map.</p>
    </dialog> : null}
  </section>;
}
