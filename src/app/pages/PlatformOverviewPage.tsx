...
import { ScenarioProcessMap } from "@/app/components/OperatingScenarioComponents";
...
function VendorHandoffs() {
  ...
  return <PlatformSection id="system-handoffs" eyebrow="How Last Mile works with the systems you already run" title="Keep each system in its lane." intro="Select a system to see its established responsibility, representative handoff, Last Mile role, and explicit boundary." tone="white" wide>
    ...
    {architectureOpen ? <Modal title="Last Mile Platform product contract" closeRef={closeButtonRef} onClose={() => setArchitectureOpen(false)}><div className="lm-architecture-text"><h3>Governed product ownership</h3><ol><li>Existing OT, industrial data, work, facilities, and service systems provide source evidence and retain their established responsibilities.</li><li>Infinit-Signal validates source evidence, time, quality, and replay before creating SSOM-conformant records and Conditions.</li><li>Singularity implements SSOM and owns accepted canonical semantic truth, topology, history, and Outcomes.</li><li>Infinit-Flow coordinates work, providers, approvals, timers, and explicit AUTO, ASSIST, and HUMAN AUTHORITY steps.</li><li>Infinit-Control presents live operating state and evidence. Flow and Control are sibling consumers of canonical context; Control is not downstream of Flow.</li></ol></div><div className="lm-platform-contract-map" aria-label="Existing systems feed Infinit-Signal and Singularity; Infinit-Flow and Infinit-Control are sibling consumers of canonical context"><section><span>01</span><strong>Existing systems</strong><p>Authoritative source and work responsibilities</p></section><ArrowRight aria-hidden="true" /><section><span>02</span><strong>Infinit-Signal</strong><p>Evidence qualification and Condition creation</p></section><ArrowRight aria-hidden="true" /><section><span>03</span><strong>Singularity / SSOM</strong><p>Accepted canonical truth and Outcomes</p></section><div className="lm-platform-contract-map__siblings"><section><span>04A</span><strong>Infinit-Flow</strong><p>Work and authority coordination</p></section><section><span>04B</span><strong>Infinit-Control</strong><p>Live state and evidence presentation</p></section></div></div></Modal> : null}
  </PlatformSection>;
}
...
function IndustryUseCases() {
  ...
  return <PlatformSection id="industry-use-cases" eyebrow="See your operation" title="The equipment changes. The operating discipline remains the same." tone="white" wide>
    ...
    <ReferenceBasis />
    {mapOpen ? <Modal title={`${active.selector} process map`} closeRef={closeButtonRef} onClose={() => setMapOpen(false)}><div className="lm-process-modal"><span>{active.selector}</span><h3>{active.problem}</h3><ScenarioProcessMap scenario={operatingScenarios[active.id as keyof typeof operatingScenarios]} /><p className="lm-reference-disclosure">{referenceDisclosure}</p><ReferenceSnapshot snapshot={snapshot} /><p>{active.result}</p></div></Modal> : null}
  </PlatformSection>;
}
