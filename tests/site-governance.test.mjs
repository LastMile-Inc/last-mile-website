...
test("every use case uses an approved process map and required disclosure", () => {
  for (const scenario of operatingScenarioList) {
    assert.ok(fs.existsSync(path.join(root, "public", scenario.processMap.src)));
    assert.match(scenario.processMap.alt, /condition-to-outcome map/i);
